...
class ConfigParserEcho(ConfigParser):
	...
	def get_par_load_save(self):
		...
		data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), self['Load_Save']['data_folder'])

		return data_path, classes, views
	
	def get_par_video_processing(self):
		save_foreground = self.getboolean('Video_Processing', 'save_foreground')
		save_overlay = self.getboolean('Video_Processing', 'save_overlay')
		validate_cropping = self.getboolean('Video_Processing', 'validate_cropping')
		save_cropped_video = self.getboolean('Video_Processing', 'save_cropped_video')
		resize = self.getboolean('Video_Processing', 'resize')
		save_frames = self.getboolean('Video_Processing', 'save_frames')
		save_pickle_echo = self.getboolean('Video_Processing', 'save_pickle_echo')
		verbose = self.getboolean('Video_Processing', 'verbose')
		side_length = self.getint('Video_Processing', 'side_length')

		return save_foreground, save_overlay, validate_cropping, save_cropped_video, resize, save_frames, \
				save_pickle_echo, verbose, side_length
	...
	def copy_conf(self, file_name, time):
		out_conf_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'out', 'used_configuration')
